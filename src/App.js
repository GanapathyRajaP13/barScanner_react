...
import { Box, Card, TextField, Typography, AppBar, Toolbar } from '@mui/material';
...
function App() {
  ...
  async function fetchTataData(data) {
    const params = data
    await api.post("/getItemCodeforTata?id=" + params, {
  ...
  async function fetchData() {
    if (scanData1 !== "") {
      ...
      await api.post("/getItemCode?id=" + params, {
        'Content-Type': 'text/plain'
      },
      ).then((response) => {
      ...
      <Box sx={displyTata === false ? app_style.appSx : app_style.appSxN}>
        <Card raised='true' sx={app_style.cardSx}>
          ...
          <CardContent>
            <Box sx={{ mt: 5 }}>
              ...
              <Box sx={{ mt: 2 }}>
                <Grid container spacing={1}>
                  <Grid item xs={6}>
                    <Box> <TextField label="Ok Count" value={okCount} disabled sx={app_style.inputSxs} /></Box>
                  </Grid>
                  <Grid item xs={6}>
                    <Box > <TextField label="Error Count" value={NokCount} disabled sx={app_style.inputSxs} /></Box>
                  </Grid>
                </Grid>
              </Box>
              <Button sx={{ mt: 3 }}>
            ...
            <Snackbar open={openErrorExist} autoHideDuration={5000} onClose={handleClose}>
              <Alert onClose={handleClose} severity="error" sx={{ width: '100%' }}>
                Customer Label code not Exist!
              </Alert>
            </Snackbar>
            <Snackbar open={openErrorAgain} autoHideDuration={3000} onClose={handleClose}>
              <Alert onClose={handleClose} severity="error" sx={{ width: '100%' }}>
        ...
        <Box>
          ...
          >
            ...
            <DialogContent>
              <DialogContentText>
                <b>Customer Label code not matched! -- (<span>{posts}</span>)</b>
              </DialogContentText>
            </DialogContent>
            <DialogActions>
      ...
      <Box sx={displyTata === true ? app_style.appSx : app_style.appSxN}>
        <Card raised='true' sx={app_style.cardSxT}>
          ...
          <CardContent>
            <Box sx={{ mt: 2 }}>
              ...
              <Box sx={{ mt: 2 }} > <TextField id="scanTata5" label="Month/Year" disabled value={scanTata5} sx={app_style.inputSxT} /></Box>
              <Box sx={{ mt: 2 }} > <TextField id="scanTata6" label="Serial No" disabled value={scanTata6} sx={app_style.inputSxT} /></Box>
              <Button sx={{ mt: 3, padding: "2px" }} onClick={resetIcon}>
                Reset
              </Button>
            </Box>
            <Snackbar open={openTata} autoHideDuration={7000} onClose={handleClose}>
              <Alert onClose={handleClose} severity="success" sx={{ width: '100%' }}>
        ...
        <Box>
          ...
          >
            ...
            <DialogContent>
              <DialogContentText>
                <b>Barcode not matched! -- (<span>{scanTata1}</span>)</b>
              </DialogContentText>
            </DialogContent>
            <DialogActions>
              <Button autoFocus onClick={handleCloseAlert}>
        ...
        <Box>
          ...
          >
            ...
            <DialogContent>
              <DialogContentText>
                <b>Barcode not Exist! -- (<span>{scanTata1}</span>)</b>
              </DialogContentText>
            </DialogContent>
            <DialogActions>
              <Button autoFocus onClick={handleCloseAlert}>
        ...
        <Box>
          ...
          >
            ...
            <DialogContent>
              <DialogContentText>
                <b>Duplicate Barcode! -- (<span>{scanTata1}</span>)</b>
              </DialogContentText>
            </DialogContent>
            <DialogActions>
              <Button autoFocus onClick={handleCloseAlert}>
